if (typeof window.customElements === 'undefined') {
    ...
    (function() {
      ...
      doc._createElement = function(tagName, typeExtension, callConstructor) {
        if (typeof typeExtension !== 'string') {
          callConstructor = typeExtension;
          typeExtension = null;
        }
        var customElements = win['customElements'];
        var element;
        if (typeExtension) {
          element = rawCreateElement.call(doc, tagName, typeExtension);
        } else {
          element = rawCreateElement.call(doc, tagName);
        }
        var definition = customElements._definitions.get((typeExtension || tagName).toLowerCase());
        if (definition) {
      ...
      doc.createElement = function(tagName, typeExtension) {
        return doc._createElement(tagName, typeExtension, true);
      }
      ...
      // patch Node.cloneNode

      var _origCloneNode = Node.prototype.cloneNode;
      Node.prototype.cloneNode = function() {
        var n = _origCloneNode.apply(this, arguments);
        var tagName = n.getAttribute('is') || n.tagName;
        var definition = customElements._definitions.get(tagName.toLowerCase());
        if (definition) {
          customElements._upgradeElement(n, definition, true);
        }
        return n;
      }
